...
s = solve_ivp(modelo,t_span=(0,10000),y0=[6,6,6],jac=jacobiano_modelo,method="Radau").y
...
#Generar la desviación estandar con la relación señal a ruido igual a 100
p=[]
pp=[]
ppp=[]
...
dsy=max_y/100
dsz=max_z/100
np.savetxt("sdx_exp1.txt",[dsx,0])
np.savetxt("sdy_exp1.txt",[dsy,0])
np.savetxt("sdz_exp1.txt",[dsz,0])
...
np.savetxt("datot_exp1.txt",tiempo)
#Elegir datos de X,Y y Z
x=[]
for i in range(0,len(datox),50):
    x.append(datox[i])
np.savetxt("datox_exp1.txt",x)
y=[]
for i in range(0,len(datoy),50):
    y.append(datoy[i])
np.savetxt("datoy_exp1.txt",y)
z=[]
for i in range(0,len(datoz),50):
    z.append(datoz[i])
np.savetxt("datoz_exp1.txt",z)
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciax_exp1.png")
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciay_exp1.png")
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciaz_exp1.png")
