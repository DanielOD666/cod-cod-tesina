...
dsy=max_y/100
dsz=max_z/100
np.savetxt("sdx_exp3.txt",[dsx,0])
np.savetxt("sdy_exp3.txt",[dsy,0])
np.savetxt("sdz_exp3.txt",[dsz,0])
...
np.savetxt("datot_exp3.txt",tiempo)
#Elegir datos de X,Y y Z
x=[]
for i in range(0,len(datox),50):
    x.append(datox[i])
np.savetxt("datox_exp3.txt",x)
y=[]
for i in range(0,len(datoy),50):
    y.append(datoy[i])
np.savetxt("datoy_exp3.txt",y)
z=[]
for i in range(0,len(datoz),50):
    z.append(datoz[i])
np.savetxt("datoz_exp3.txt",z)
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciax_exp3.png")
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciay_exp3.png")
...
plt.grid(True)
plt.legend()
plt.savefig("diferenciaz_exp3.png")
